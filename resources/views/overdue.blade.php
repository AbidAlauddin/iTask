        ...
        <div class="w-full">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-4 border rounded-lg shadow-sm bg-white dark:bg-gray-800"
                     ...
                     data-list-id="{{ $task->category_id }}" {{-- Menggunakan category_id milik task secara langsung untuk menghindari error jika $category null --}}
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        {{-- Menyesuaikan link edit agar lebih robust jika $category (dari grup) null, dengan asumsi route bisa menerima ID kategori task atau model Category task --}}
                        <a href="{{ $category ? route('lists.tasks.edit', [$category, $task]) : ($task->category_id ? route('lists.tasks.edit', [$task->category_id, $task]) : '#') }}" class="task-title text-gray-900 dark:text-gray-100 hover:underline {{ $task->completed ? 'line-through text-gray-400' : '' }}">{{ $task->title }}</a>
                    </div>
